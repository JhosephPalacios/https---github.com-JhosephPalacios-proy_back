...
const Principal = () => {
    ...
    function crearCarta(index, libro) {
        return (
            <>
    ...
    function crearCartaUSER(index, libro) {
        return (
            <>
                <div className="carta" key={index}>
		            <div className="contenido">
			            <h2 className='Titulo_card'>{libro.titulo}</h2>
                    	<p className='Fecha_card'>Fecha limite: {new Date(libro.fechaentrega).toLocaleDateString("es-ES", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit" })} User: {libro.usuario.split("@")[0]}</p>
		            </div>
                    <div className="imagen">
                        <img src={libro.portada} alt="/media.png" className="icono_default" />
                    </div>

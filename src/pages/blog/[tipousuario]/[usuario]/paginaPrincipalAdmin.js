              ...
              {reservas.slice(-2).map((reserva, index) => (
                <div className="carta" key={index}>
                  <div className="contenido">
                    <h2 className='Titulo_card'>{reserva.titulo}</h2>
                    <p className='Fecha_card'>{new Date(reserva.fecha).toLocaleDateString("es-ES", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit" })} User: {reserva.usuario.split("@")[0]}</p>
                    <p className='User_card'></p>
                  </div>
          ...
          <div class="titulo_seccion">Los más pedidos</div>
            <div class="cartas_fila">
              {stats.slice(0, 2).map((libro, index) => (
                <div className="carta" key={index}>
                  <div className="contenido">
                    <h2 className='Titulo_card'>{libro.titulo}</h2>
            ...
            <Link href={`/blog/admin/${usuario}/paginaProximosVencer`} class="ver-todo">Ver todo</Link>
          </div>
        </>
      } />
    </>
